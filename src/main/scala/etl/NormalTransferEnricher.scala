...
object NormalTransferEnricher extends BaseEnricher {
  ...
  private def processBlockRange(startBlockNumber: Int, endBlockNumber: Int): Unit = {
  ...
  override def fetchAndEnrichData(): Unit = {
    ...
    try {
      ...
      batchedBlockNumbers.foreach { batch =>
        batch.foreach { startBlockNumber =>
          val endBlockNumber = startBlockNumber + blockSize - 1
          processBlockRange(startBlockNumber, endBlockNumber)
          Thread.sleep(3000)
        }
